...
# def file_display(request, file_id):
#     file_obj = File.objects.get(id=file_id)
#     file_path = file_obj.file.url
#     mime_type, _ = mimetypes.guess_type(file_path)
#     print(file_path)
#     file_type = mime_type.split('/')[0] if mime_type else None
#     return render(request, 'openfile.html', {'file_obj': file_obj, 'file_type': file_type})
def file_display(request, file_id):
    file_obj = get_object_or_404(File, pk=file_id)
    file_path = file_obj.file.url
    mime_type, _ = mimetypes.guess_type(file_path)
    file_type = mime_type.split('/')[0] if mime_type else None
    return render(request, 'googledriveapp/openfile.html', {'file_obj': file_obj, 'file_type': file_type})
